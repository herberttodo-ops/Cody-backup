...
3. **Set up automated renewal immediately** via `hermes cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#___LONG_STRING___` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
