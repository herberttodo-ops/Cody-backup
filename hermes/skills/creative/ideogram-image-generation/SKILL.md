...
- **Auto-post script**: `scripts/optirfp_autopost.py` — Full pipeline: generate → upload → post
- **Random content generator**: `scripts/optirfp_post_generator.py` — Content pool + calls autopost (for cron jobs)
- Compositor script: `scripts/ideogram_logo_compositor.py` — Complete working implementation
- Reference: `references/upload-service-status.md` — **Current status of upload services** (catbox.moe working, Google Drive broken for Buffer)
- Auto-post script: `scripts/optirfp_autopost.py` - Full pipeline: generate → upload → post
...
- Reference: `references/upload-service-status.md` - **Current status of upload services** (catbox.moe working, Google Drive broken)
...
### Cron Job Workflow (Ideogram + Logo + Manual Upload)

This is the PROVEN reliable pattern. Do NOT attempt automated uploads - they fail.

```python
# 1. Generate image with logo composited using the CLI
cd ~/.openclaw/workspace && python3 ~/.hermes/skills/creative/ideogram-image-generation/scripts/ideogram_logo_compositor.py --headline "YOUR HEADLINE" --topic winning --aspect square

# 2. Verify quality with vision_analyze (run interactively if possible)
# vision_analyze on the GENERATED image at: ~/.hermes/generated_images/ideogram_logo_*.png

# 3. Output manual upload package - do NOT attempt uploads in cron
```

**Expected Output Format:**
```
📁 Image: /home/herby/.hermes/generated_images/ideogram_logo_YYYYMMDD_HHMMSS.png
...
🏷️ HASHTAGS: #RFP #B2B #ProposalWriting

📋 ACTION FOR ANDREW:
Upload this image to Buffer → OptiRFP LinkedIn channel, paste the text above, and schedule.
```

**WHY THIS PATTERN WORKS:**
- Ideogram generates text + background = reliable text quality
- Logo compositing is deterministic (exact brand logo)
- Manual upload avoids all temporary hosting failures
- No broken LinkedIn previews from bad URLs

**DO NOT USE** (proven to fail):
- Google Drive URLs → Bad LinkedIn previews
- catbox.moe → 500 errors  
- file:// URLs → Buffer rejects them
- Direct Buffer MCP with local paths → validation errors

### Upload Service History

These previously worked but now fail:
- **catbox.moe**: Was working Aug 2026, now returns 500 errors
- **transfer.sh**: Connection refused
- **0x0.st**: Uploads disabled due to spam
- **Google Drive**: Works for storage, NOT for hot-linked image URLs in Buffer
