...
**Slot:** 23:00 UTC / 7:00 PM EDT (2026-09-25)  
**Buffer Cap:** 8/10 scheduled (2 slots remaining)

---

**Title:** I Traced Dracula's Bloodline to a Village That Doesn't Exist Anymore  
**Entity:** Dracula (Tier 1 folklore — proven 1.1K views)  
**Variant:** A (direct first-person hook)  
**Tier:** 1  

**Production Summary:**
- Story: 165 words, entity in first sentence, Transylvania/Bihor Romania setting
- Voice: ElevenLabs Adam (POYO TTS)
- Duration: 58 seconds
- Scenes: 6 Gammell-style ink wash illustrations
- File: `output/FINAL_25_dracula_bloodline.mp4` (152MB)

**Buffer Post:**
- Post ID: `6ab6a0168fd8a73566e30d84`
- Status: `scheduled`
- Due At: `2026-09-25T23:00:00.000Z` (7:00 PM EDT)

**A/B Tracker:** Logged variant A successfully.

---

**Issues Encountered & Fixes Applied:**

1. **Buffer rejected standard Google Drive URL** ("Video could not be read from its URL.")
   - *Fix:* Switched to `drive.usercontent.google.com/download?id=...&export=download&confirm=t` — this returns `Content-Type: video/mp4` directly, bypassing the virus-scan HTML gate that trips Buffer's parser.
   - **Recommendation:** Update `pipeline.py`, `buffer_dual_account.py`, and `references/buffer-publishing.md` to use the `drive.usercontent.google.com` URL pattern with `&confirm=t`.

2. **BunnyCDN 403** — The `tales-untold` storage zone is not publicly accessible via CDN. Public pull requires zone config changes.

3. **YouTube OAuth expired** — `~/.config/youtube_credentials.json` failed refresh with `invalid_grant`. Direct YouTube upload is blocked until re-authenticated.
